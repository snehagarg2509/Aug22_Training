...
#define PORT 5050 // Port number for the server
...
void process_cdr1(const char *filename, const char *msisdn, CustomerData *data);
int find_or_create_customer(CustomerSummary **customers, int *customer_count, const char *brand, const char *id);
void process_cdr_by_common_customer(const char *filename, CustomerSummary **customers);
void process_cdr_by_id(const char *filename, const char *mmc, OperatorData *data);
int find_or_create_operator(OperatorSummary **operators, int *operator_count, const char *brand, const char *id);
void process_cdr_by_common(const char *filename, OperatorSummary **operators);
...
void customerBilling(int client_socket) {
    ...
    switch (billingChoice) {
        case USER_BILLING_ACTION_MSISDN:
            ...
            process_cdr1("user.txt", msisdn, &data); // Process the CDRs for the given MSISDN
            sprintf(response,
        "#Customer Data Base:\n"
        "Customer ID: %s (%s)\n"
        "\t* Services within the mobile operator *\n"
        ...
        case USER_BILLING_ACTION_CSFILE:
            process_cdr_by_common_customer("user.txt", customers);
...
void interoperatorBilling(int client_socket) {
    ...
    switch (billingChoice) {
        case USER_BILLING_ACTION_OPID:
            ...
            process_cdr_by_id("user.txt",operatorID,&data);
        ...
        case USER_BILLING_ACTION_OPFILE:
            process_cdr_by_common("user.txt", operators);
